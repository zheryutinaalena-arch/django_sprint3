...
def test_models_translated(n_model, n_verbose, n_verbose_plural):
    ...
    found_model = [
        model for model in models if model._meta.object_name == n_model
    ]
    ...
    assert (
        found_model._meta.verbose_name == n_verbose
    ...
    assert (
        ...
        n_model=n_model, param_name="verbose_name_plural"
    )
